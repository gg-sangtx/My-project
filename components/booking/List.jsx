...
class List extends React.Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      dataSort: [{key: 'ASC', value: '予約日昇順'},{key: 'DESC', value: '予約日降順'}],
  ...
  getListData() {
    ...
    if (this.state.viewList == 'calendar') {
      ...
      params.is_except_canceled_status = 1;
      params.limit = 0;
    } else {
      if(this.props.goBack == true) {
    ...
    if (this.state.viewList == 'calendar') {
      ...
      Booking.actions.list.request(params).then(res => {
        ...
        this.props.dispatch({type: 'BOOKING_RESET_GO_BACK'});
      }).catch(err => {
  ...
  search(dataSearch) {
    ...
    this.props.dispatch({type: 'UPDATE_BOOKING_SEARCH', dataSearch: this.state.dataSearch});
    this.setState({
      ...this.state
    })
    this.getListData();
  ...
  onChange(name, e) {
    ...
    if(name == 'sort') {
      ...
      this.state.dataSearch = Search;
      this.props.dispatch({type: 'UPDATE_BOOKING_SEARCH', dataSearch: Search});
  ...
  handlePageClick(page) {
    ...
    this.props.dispatch({type: 'UPDATE_BOOKING_PAGE', pageData: page.selected});
    this.getListData();
  }
