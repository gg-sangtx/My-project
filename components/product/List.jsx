...
class List extends Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      data: [],
      dataSort: [{key: 'PRODUCT_CODE', value: '物販商品コード'}, {key: 'NAME', value: '物販商品名'}, {key: 'PRICE', value: '定価'}, {key: 'REAL_PRICE', value: '販売価格'}],
      dataSearch: {},
      sort: 'PRODUCT_CODE',
      messageDelete: '',
      modalIsOpen: false,
  ...
  componentDidMount() {
    if(this.props.goBack == true) {
      ...
      this.state.sort = this.props.dataSearch.sort || 'PRODUCT_CODE';
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.goBack == true) {
      ...
      this.state.sort = nextProps.dataSearch.sort || 'PRODUCT_CODE';
      this.setState({...this.state})
    }
  }
  ...
  getListData() {
    ...
    if (dataSearch && dataSearch.jan_code) {
      params.jan_code = dataSearch.jan_code
    }
    ...
    if (dataSearch && dataSearch.real_price_from) {
      params.real_price_from = dataSearch.real_price_from
    }
    if (dataSearch && dataSearch.real_price_to) {
      params.real_price_to = dataSearch.real_price_to
    }
    if (dataSearch && dataSearch.product_skus_product_code) {
      params.product_skus_product_code = dataSearch.product_skus_product_code
    }
    if (dataSearch && dataSearch.sort) {
      params.sort_field = dataSearch.sort
    } else {
      params.sort_field = 'PRODUCT_CODE'
    }

    params.sort_type = 'ASC';

    Product.actions.list.request(params).then(res => {
      if(res && res.data) {
        ...
        this.props.dispatch({type: 'UPDATE_PRODUCT_LIST', staff: Data.data});
        this.props.dispatch({type: 'PRODUCT_RESET_GO_BACK'});
      } else {
        this.state.data = [];
        ...
        this.props.dispatch({type: 'UPDATE_PRODUCT_LIST', staff: []});
        this.props.dispatch({type: 'PRODUCT_RESET_GO_BACK'});
      }
    }).catch(err => {
  ...
  search(dataSearch) {
    ...
    this.props.dispatch({type: 'UPDATE_PRODUCT_SEARCH', dataSearch: Search});
    this.props.dispatch({type: 'UPDATE_PRODUCT_PAGE', pageData: 0});
    this.setState({
      ...this.state
    })
    this.getListData();
  ...
  handlePageClick(page) {
    ...
    this.props.dispatch({type: 'UPDATE_PRODUCT_PAGE', pageData: page.selected});
    this.getListData();
  }
  ...
  openModal(id) {
    ...
    if (dataFilter.length > 0) {
      this.state.messageDelete = dataFilter[0].name + msg.confirmDeleteProduct;
    }
  ...
  deleteItem() {
    ...
    Product.actions.delete.request({id: this.state.deleteId}).then(res => {
      Toastr(dataFilter[0].name + msg.deleteProduct, 'success');
      if(this.checkLastItem()) {
        this.state.pageData.number = this.state.pageData.number - 1;
        this.props.dispatch({type: 'UPDATE_PRODUCT_PAGE', pageData: this.state.pageData.number});
      }
      this.closeModal();
      this.getListData();
    }).catch(err => {
    ...
    return (
      <div id='content-list-manager'>
        <div className='title-block'>
          <h2 className='heading-2'>物販商品情報 <small>一覧</small></h2>
          <Link className='btn-addnew' to='/products/create'>新規登録</Link>
        </div>
